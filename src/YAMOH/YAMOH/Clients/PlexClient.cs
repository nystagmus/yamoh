...
public class PlexClient
{
    ...
    public async Task<FileInfo?> DownloadPlexImageAsync(string urlStub)
    {
        ...
        catch (Exception ex)
        {
            ...
            _logger.LogError(ex, "Failed to download Plex image: {FullUrl}", RemovePlexAuthToken(fullUrl));
            return null;
        }
    }
    ...
    public async Task<GetMetadataChildrenResponseBody?> GetMetadataChildrenAsync(int ratingKey)
    {
        ...
        {
            // Log error (strip the params because the only param is our auth key
            _logger.LogError(ex, "Failed to download Plex Metadata Children: {FullUrl}", RemovePlexAuthToken(fullUrl));
            return null;
        }
    }
    ...
    public async Task<List<PlexLabel>?> GetLabelsForPlexIdAsync(int plexId)
    {
        ...
        try
        {
            var response = await _httpClient.GetFromJsonAsync<PlexMetadataResponse>(fullUrl,
                new JsonSerializerOptions(JsonSerializerDefaults.Web));
        ...
        {
            _logger.LogError(ex, "Failed to fetch labels for PlexId: {FullUrl}", RemovePlexAuthToken(fullUrl));
            return null;
        }
    }
